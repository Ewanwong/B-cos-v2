...
class RobertaEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.LayerNorm = norm(config.hidden_size, eps=config.layer_norm_eps)
...
class RobertaSelfAttention(BcosModelBase):
...
class RobertaSdpaSelfAttention(RobertaSelfAttention):
    ...
    def forward(
        ...
        logger.warning_once(
            "RobertaSdpaSelfAttention is not yet b-cosified. "
        )
...
class RobertaEncoder(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, "bcos") and config.bcos and hasattr(config, "different_b_per_layer") and config.different_b_per_layer and hasattr(config, "b_list") and config.b_list is not None:
            assert len(config.b_list) == config.num_hidden_layers, "Length of b_list should be equal to num_hidden_layers"
            ...
            self.layer = nn.ModuleList([RobertaLayer(config_list[i]) for i in range(config.num_hidden_layers)])
        else:
...
class RobertaPreTrainedModel(PreTrainedModel, BcosModelBase):
    """
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if not hasattr(self.config, "bce") or not self.config.bce:
                ...
                lm_loss = loss_fct(sigmoid(shifted_prediction_scores), targets)
...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        ## bcos
        self.bcos = config.bcos if hasattr(config, "bcos") else False
        if self.bcos and hasattr(config, "b"):
            linear = partial(BcosLinear, b=config.b)
        else:
            linear = nn.Linear
