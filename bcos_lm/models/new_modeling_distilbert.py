...
class MultiHeadSelfAttention(BcosModelBase):
    def __init__(self, config: PretrainedConfig):
        ...
        if self.bcos and self.bcos_attention:
            self.q_lin = BcosLinear(config.dim, config.dim, b=config.b)
            self.k_lin = BcosLinear(config.dim, config.dim, b=config.b)
            self.v_lin = BcosLinear(config.dim, config.dim, b=config.b)
        ...
        if labels is not None:
            ...
            if not hasattr(self.config, "bce") or not self.config.bce:
                ...
                if not hasattr(self.config, "relative_logits") or not self.config.relative_logits:
                    ...
                    mlm_loss = loss_fct(sigmoid(prediction_logits+math.log(1/(self.config.vocab_size-1))), targets)
