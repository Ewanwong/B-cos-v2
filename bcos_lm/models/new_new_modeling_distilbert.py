...
class Embeddings(nn.Module):
    def __init__(self, config: PretrainedConfig):
        ...
        self.LayerNorm = norm(config.dim, eps=1e-12)
...
class MultiHeadSelfAttention(BcosModelBase):
    def __init__(self, config: PretrainedConfig):
        super().__init__()
        self.config = config
        ## bcos
        ...
        if self.bcos:
            ...
            self.out_lin = BcosLinear(config.dim, config.dim, b=config.b)
        else:
    ...
    def forward(
        ...
        ## bcos
        context = self.dynamic_multiplication(weight=weights, input=v)
...
class DistilBertFlashAttention2(MultiHeadSelfAttention):
    ...
    def forward(
        ...
        logger.warning_once(
            "FlashAttention2 is not yet b-cosified. "
        )
...
class TransformerBlock(nn.Module):
    def __init__(self, config: PretrainedConfig):
        ...
        self.sa_layer_norm = norm(normalized_shape=config.dim, eps=1e-12)

        self.ffn = FFN(config)
        self.output_layer_norm = norm(normalized_shape=config.dim, eps=1e-12)
...
class Transformer(nn.Module):
    def __init__(self, config: PretrainedConfig):
        ...
        if hasattr(config, "bcos") and config.bcos and hasattr(config, "different_b_per_layer") and config.different_b_per_layer and hasattr(config, "b_list") and config.b_list is not None:
            assert len(config.b_list) == config.n_layers, "Length of b_list should be equal to num_hidden_layers"
            config_list = [copy.deepcopy(config) for _ in range(len(config.b_list))]
            for i in range(len(config.b_list)):
                config_list[i].b = config.b_list[i]
            self.layer = nn.ModuleList([TransformerBlock(config_list[i]) for i in range(config.n_layers)])
        else:
...
class DistilBertPreTrainedModel(PreTrainedModel, BcosModelBase):
    """
    ...
    ## bcos
    @classmethod
    def load_from_pretrained(cls, model_name_or_path, config=None, **kwargs):
        def is_bcos(config):
        ...
        if not is_bcos(config) and not is_bcos(orig_config):
            ...
            for name, params in bcos_model_dict.items():
                if name in conventional_model_dict:
                    bcos_model_dict[name].data.copy_(conventional_model_dict[name].data)
                elif name.replace(".linear.", ".") in conventional_model_dict:
                    bcos_model_dict[name].data.copy_(conventional_model_dict[name.replace(".linear.", ".")].data)
                else:
                    raise ValueError(f"Parameter {name} not found in conventional model")
...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        ...
        self.distilbert = DistilBertModel(config)
        self.pre_classifier = linear(config.dim, config.dim)
        self.classifier = linear(config.dim, config.num_labels)
